 ...
 * bHaveChargeOrTypePerturbed handles the control flow. 如果仅仅是质量被扰动，
 * 这里没有什么特别的事情要做，但如果电荷或类型中有一个被扰动，那么实现就需要
 * 为电荷和类型定义B状态，并且不优化只有一个变化的情况。当FEP不活跃时，或者当没
 *  有质量/电荷/类型扰动时，B态的参数向量在atoms2md()中是未定义的。当LJ-PME不活
 * 跃时，LJ-PME的参数向量同样未定义。这是因为bHaveChargeOrTypePerturbed处理控制流。*/
