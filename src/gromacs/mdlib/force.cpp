...
#pragma omp parallel for num_threads(nthreads) schedule(static)
            ...
            if (computePmeOnCpu)
            {
                ...
                if (numTpiAtoms_ == 0 || stepWork.stateChanged)
                {
                    ...
                    //std::cout << "The rank is " << commrec->sim_nodeid << "   The force is " << forceWithVirial->force_[0][0] << "      " << forceWithVirial->force_[0][1] << "     " << forceWithVirial->force_[0][2] << std::endl;
                    //std::cout << "The rank is " << commrec->sim_nodeid << "   The virial is " << ewaldOutput.vir_q[0][0] << "      " << ewaldOutput.vir_q[1][1] << "     " << ewaldOutput.vir_q[2][2] << std::endl;
